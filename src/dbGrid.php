...
class dbGrid {
	...
	//constructor
	public function __construct($db_link, $query, $col_count = 1, $lines_per_page = 10, $title_spanned = 0, $page_public = "page", $max_page_link = 10) {
		global $$page_public;
        $this->db_link = $db_link;
		$this->max_page_link = $max_page_link;
		$this->page_public = $page_public;
		$this->query = $query;
		$this->page_number = $_GET[$this->page_public];
		if ($this->page_number == "") 
            $this->page_number = 1;
		$this->tb_width = "100%";
	...
	function _do_submit() {
		for ($i = 0;$i <= $this->col_count;$i++) {
			if (($pos = strpos($this->col_array[$i], "#c#")) !== false) {
				$j = $pos + 3;
				while (substr($this->col_array[$i], $j, 1) != "#") $j++;
				...
				while (($this->query_row = mysqli_fetch_array($this->query_table)) && ($row <= $this->lines_per_page)) {
					...
					$public_name = "c" . $i . "_" . $pk_code;
					$value = $_POST[$public_name];
					if ($value == "") $value = "0";
					$this->query('
	...
	function query_string() {
		...
		if (substr($qstr, 0, 5) == "page=") {
			$and_pos = strpos($qstr, "&");
			if ($and_pos === false) $new_qstr = "";
			else $new_qstr = substr($qstr, $and_pos + 1, strlen($qstr));
		}
		else {
			$page_pos = strpos($qstr, "&page=");
			if ($page_pos !== false) {
				...
				if ($and_pos === false) $new_qstr = $first_part;
				else $new_qstr = $first_part . substr($second_part, $and_pos, strlen($second_part));
			}
			else $new_qstr = $qstr;
		}
		return ($new_qstr);
	...
	function initialize_grid() {
		if ($this->submit_label == "") $this->submit_label = "Submit";
		$found_form = false;
		...
		if ($this->has_form && $_POST["_submit"] != "") $this->_do_submit();
	...
	function show_page_link($style = '', $class = '') {
		...
		if ($max > $this->max_page) $max = $this->max_page;
		for ($page = $min;$page <= $max;$page++) {
			if ($page != $this->page_number) {
				$qs = $this->query_string();
				$qstr = $this->caller_file_name . '?' . $qs . ($qs == "" ? "" : "&") . $this->page_public . '=' . $page;
				if ($this->add_qstr) $qstr .= '&' . $this->add_qstr;
				echo '<a href="' . $qstr . '"';
	...
	function show_prior($message, $style = '', $class = '', $show_disabled = false) {
		if ($this->have_prior) {
			$qs = $this->query_string();
			$qstr = $this->caller_file_name . '?' . $qs . ($qs == "" ? "" : "&") . $this->page_public . '=' . ($this->page_number - 1);
			if ($this->add_qstr) $qstr .= '&' . $this->add_qstr;
			echo '<a href="' . $qstr . '"';
			if ($style) echo ' style="' . $style . '"';
			if ($class) echo ' class="' . $class . '"';
		...
		else if ($show_disabled) echo $message;
	}
	...
	function show_content($content, $return = false) {
		$this->row_num = 1 + ($this->page_number - 1) * $this->lines_per_page;
		if ($this->title_spanned) mysqli_data_seek($this->query_table, $this->col_count * ($this->row_num - 1));
		else mysqli_data_seek($this->query_table, $this->row_num - 1);
		$row = 0;
	...
	function _show_content($content, $col_num = - 1, $return = false) {
		...
		while (($pos = strpos($content, '#g#')) !== false) {
			$i = $pos + 3;
			while (substr($content, $i, 1) != "#") $i++;
			$field_name = substr($content, $pos + 3, $i - $pos - 3);
			if ($this->query_row = mysqli_fetch_array($this->query_table)) $content = substr($content, 0, $pos) . $this->query_row[$field_name] . substr($content, $i + 1, strlen($content));
			else {
				$content = "";
				$empty = true;
			}
		...
		while (($pos = strpos($content, '#D#')) !== false) {
			...
			while (substr($content, $i, 1) != "#") $i++;
			$field_name = substr($content, $pos + 3, $i - $pos - 3);
			if ($this->query_row[$field_name] == "") $date_str = "";
			else $date_str = dateTimeFormat::str2date($this->query_row[$field_name]);
		...
		while (($pos = strpos($content, '#i#')) !== false) {
			...
			else $content = substr($content, 0, $pos) . substr($content, $i + 1, strlen($content));
		}
	...
	function _show_td($col, $start = 1, $col_span = 1) {
		if ($start != 0) {
			echo '<td';
			if ($this->td_width_array[$col]) echo ' width="' . $this->td_width_array[$col] . '"';
			if ($this->td_height_array[$col]) echo ' height="' . $this->td_height_array[$col] . '"';
			if ($this->td_align_array[$col]) echo ' align="' . $this->td_align_array[$col] . '"';
			if ($this->td_valign_array[$col]) echo ' valign="' . $this->td_valign_array[$col] . '"';
			if ($this->td_style_array[$col]) echo ' style="' . $this->td_style_array[$col] . '"';
			if ($this->td_class_array[$col]) echo ' class="' . $this->td_class_array[$col] . '"';
			if ($this->td_ltr_array[$col]) echo ' dir="ltr"';
			if ($this->td_class) echo ' class="' . $this->td_class . '"';
			if ($col_span != 1) echo ' colspan="' . $col_span . '"';
			echo '>';
		}
		else {
			echo '</td>';
	...
	function show_grid() {
		...
		if ($this->show_header) {
			...
			if ($this->title_spanned) {
				...
				if ($this->tr_head_color) echo '<font color="' . $this->tr_head_color . '">';
				echo $this->title_array[0];
				if ($this->tr_head_color) echo '</font>';
				$this->_show_td("", 0);
			}
		...
		$even = false;
		$row = 1;
		if ($this->title_spanned == 1) mysqli_data_seek($this->query_table, $this->col_count * ($this->row_num - 1));
		else mysqli_data_seek($this->query_table, $this->row_num - 1);
		...
		if ($this->has_form) {
			...
			if ($this->submit_class) echo ' class="' . $this->submit_class . '"';
			if ($this->submit_style) echo ' class="' . $this->submit_style . '"';
			echo '>';
			echo '</td></tr>';
		}
		$this->_show_table(0);
		if ($this->has_form) echo '</form>';
	}
}
?>
